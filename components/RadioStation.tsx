...
export default function RadioStation() {
  ...
  return (
    <div 
      className="w-full h-screen bg-zinc-400 text-zinc-800 flex items-center justify-center p-4 selection:bg-amber-500 selection:text-white overflow-hidden relative"
    ...
    >
      ...
      <div className="flex flex-col md:flex-row gap-8 w-full max-w-5xl relative z-10 bg-zinc-200 p-8 rounded-xl shadow-2xl border-t border-l border-white/60 border-b-zinc-400 border-r-zinc-400">
         
         {/* Main Interface */}
         <div className="flex-1 flex flex-col gap-6">
            
            {/* LCD Header */}
            <div className="bg-zinc-800 border-4 border-zinc-900 border-b-zinc-700/50 p-6 rounded-md shadow-[inset_0_0_30px_rgba(0,0,0,0.8)] flex flex-col justify-between h-48 relative overflow-hidden font-mono">
                ...
                <div className="flex justify-between items-start text-amber-500/50 text-[10px] uppercase tracking-widest relative z-10">
                ...
                <div className={`text-center font-bold text-[5rem] leading-none tracking-widest flex items-baseline justify-center gap-4 transition-colors duration-500 relative z-10 ${power ? 'text-amber-500 drop-shadow-[0_0_12px_rgba(245,158,11,0.7)]' : 'text-zinc-900 drop-shadow-none'}`}>
                    {power ? frequency.toFixed(1) : '--.-'}
                    <span className="text-2xl opacity-50">MHz</span>
                </div>

                <div className="flex justify-between items-end text-amber-500/70 text-[10px] uppercase font-bold tracking-widest relative z-10">
            ...
            {/* Tuning Dial Strip */}
            <div className={`h-24 w-full border border-zinc-900 bg-zinc-800 rounded-sm shadow-inner relative overflow-hidden transition-opacity duration-1000 ${power ? 'opacity-100' : 'opacity-30'} font-mono`}>
            ...
            <div className="flex-1 border-t-2 border-l-2 border-white/60 border-b-2 border-r-2 border-zinc-400 p-1 bg-zinc-300 min-h-[220px] rounded text-zinc-800 font-mono">
                <div className="h-full border border-black/10 p-4 flex flex-col rounded bg-zinc-200">
                    {!power ? (
                        ...
                        <div className="flex w-full h-full gap-4">
                            ...
                            <div className="w-32 flex flex-col justify-between shrink-0 gap-2">
                                 <div className="border border-zinc-300 p-4 bg-zinc-100 flex flex-col gap-2 rounded text-center">
                                 ...
                                 <div className="flex flex-col gap-2">
                                    <button onClick={togglePlayMode} className="w-full py-4 flex items-center justify-center border border-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors bg-zinc-100 rounded shadow-sm">
                                         {isPlaying ? <Pause size={20}/> : <Play size={20}/>}
                                    </button>
                                    <button onClick={skipForward} className="w-full py-4 flex items-center justify-center border border-zinc-400 hover:bg-zinc-800 hover:text-white transition-colors bg-zinc-100 rounded shadow-sm">
                                         <SkipForward size={20}/>
                                    </button>
                                 </div>
                            </div>
         ...
         {/* Hardware Controls */}
         <div className="w-full md:w-64 flex flex-col gap-6 shrink-0 justify-between font-sans">
            
            <div className="border-t-2 border-l-2 border-white/60 border-b-2 border-r-2 border-zinc-400 p-6 flex flex-col items-center gap-10 bg-zinc-200 flex-1 justify-center relative touch-none pointer-events-auto select-none rounded box-border shadow-inner"
               onPointerDown={(e) => {
            ...
            >
                ...
                >
                    ...
                    <div className="absolute inset-x-8 inset-y-8 rounded-full border border-zinc-400 bg-gradient-to-br from-zinc-100 to-zinc-300 shadow-[inset_0_2px_5px_rgba(255,255,255,0.7)]"></div>
                    <div className="w-full h-full rounded-full relative z-10 transition-transform duration-75 pointer-events-none" style={{ transform: `rotate(${knobRot}deg)` }}>
                        <div className={`absolute top-3 left-1/2 -translate-x-1/2 w-3 h-8 rounded-full shadow-inner transition-colors border border-black/10 ${power ? 'bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.6)]' : 'bg-red-800 shadow-[inset_0_2px_5px_rgba(0,0,0,0.4)]'}`}></div>
                    </div>
                </div>
                <span className="text-[9px] uppercase tracking-widest font-bold opacity-50 pointer-events-none text-zinc-800">DRAG TO TUNE</span>
            </div>

            <div className="border-t-2 border-l-2 border-white/60 border-b-2 border-r-2 border-zinc-400 rounded p-6 flex flex-col items-center gap-6 bg-zinc-200 shadow-inner">
                <span className="text-[10px] uppercase tracking-widest font-bold opacity-60 w-full text-center border-b border-black/10 pb-2 text-zinc-800">SYSTEM POWER</span>
                <button 
                   onClick={togglePower}
                   className={`w-20 h-20 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${power ? 'border-red-500 bg-red-100 text-red-600 shadow-[0_0_20px_rgba(239,68,68,0.4),inset_0_3px_10px_rgba(255,255,255,0.9)] hover:shadow-[0_0_30px_rgba(239,68,68,0.6)]' : 'border-zinc-300 bg-zinc-300 text-zinc-500 shadow-[2px_5px_10px_rgba(0,0,0,0.2),inset_0_3px_10px_rgba(255,255,255,0.9)] hover:bg-zinc-200'}`}
                >
                    <Power size={32} />
                </button>
            </div>
